...
public class GuildJoinCommand implements ISubCommand {
  ...
  public void execute(CommandSender commandSender, String[] args) {
    ...
    if (guild.getPlayers().size() >= guildModule.getGuildHandler().getMaxPlayers()) {
      sender.sendMessage(Lang.GUILD_JOIN_TOO_MANY_PLAYERS.toComponentWithPrefix(Map.of("%guild%", name)));
      return;
    }

    if (!guild.getInvitedPlayers().contains(sender.getUniqueId())) {
      sender.sendMessage(Lang.GUILD_JOIN_NOT_INVITED.toComponentWithPrefix(Map.of("%guild%", name)));
      return;
    }

    sender.sendMessage(Lang.GUILD_JOIN_PLAYER_ACKNOWLEDGE.toComponentWithPrefix(Map.of("%guild%", name)));
