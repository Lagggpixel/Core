...
public class BazaarInstantBuyGui extends Gui {
    ...
    public BazaarInstantBuyGui(Player opener, BazaarSubItem item, int customAmount) {
        ...
        this.addItem(31, MiscUtil.buildBackButton("&7To " + name));
        ...
        ItemStack buyOne = new ItemBuilder(icon.clone()).setDisplayName(ChatColor.GREEN + "Buy only " + ChatColor.YELLOW + "one" + ChatColor.GREEN + "!").setLore(Arrays.asList(MiscUtil.buildLore(
        ...
        ItemStack buyStack = new ItemBuilder(icon.clone()).setDisplayName(ChatColor.GREEN + "Buy a stack!").setLore(Arrays.asList(MiscUtil.buildLore(
                "&8" + name + "\n\nAmount: &a" + skyblockItem.getMaxStackSize() + "&7x\n\n" + (item.getLowestSellPrice() > 0.0 ? "Per Unit: &6" + MiscUtil.formatDouble(item.getLowestSellPrice()) + " coins\n" + "Price: &6" + MiscUtil.formatDouble(item.getLowestSellPrice() * skyblockItem.getMaxStackSize()) + " coins" : "&cNo sell offers!") +
        ...
        ItemStack buyInventory = new ItemBuilder(ChatColor.GREEN + "Fill my inventory!", Material.CHEST).setLore(
                Arrays.asList(MiscUtil.buildLore(
                        "&8" + name + "\n\n" + (canFillInventory ? "Amount: &a" + MiscUtil.formatInt(possibleAmountToCarry) + "&7x\n\nPer Unit: &6" + MiscUtil.formatDouble(item.getLowestSellPrice()) + " coins\nPrice: &6" + MiscUtil.formatDouble(item.getLowestSellPrice() * possibleAmountToCarry) + " coins" : "No one is selling this item!") + "\n\n" +
